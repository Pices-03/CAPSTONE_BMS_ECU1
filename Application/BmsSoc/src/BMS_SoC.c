...
* Component : BMS_SoC
* Module : BMS_SoC.c
* Description : Source file for State of Charge (SoC) estimation module with current mapping.
*               Supports simulation of both charging and discharging from unidirectional current.
*==================================================================================================*/
...
/* ========== THÊM CÁC BIẾN DEBUG NÀY ========== */
static volatile float32 g_dbg_deltaTime = 0.0f;      // Lưu delta time vừa tính
static volatile uint32  g_dbg_elapsedTicks = 0U;    // Lưu số tick đã trôi qua
static volatile uint32  g_dbg_timeNow = 0U;         // Lưu thời gian hiện tại
static volatile uint32  g_dbg_timePrev = 0U;        // Lưu thời gian trước đó
static volatile uint32  g_dbg_callCount = 0U;       // Đếm số lần gọi
/* ============================================== */
...
/**
 * @brief Update SoC using Coulomb counting with mapped current direction.
 * @param rawCurrent_mA Raw current from INA219 (mA) - always positive
 * @param deltaTime_sec Elapsed time since last update (seconds)
 */
Std_ReturnType BMS_SoC_Update(float32 rawCurrent_mA)
{
	Std_ReturnType Status = E_OK;
    float32 deltaTime_sec;
    float32 delta_Ah;
    float32 mappedCurrent_A;
    ...
    else
    {
		...
		delta_Ah = mappedCurrent_A * deltaTime_sec / 3600.0f;

		/* Update remaining capacity (discharge subtracts, charge adds) */
		BMS_SoC_State.RemainingCapacityAh -= delta_Ah;

		/* Apply limits (prevent negative or overcharged) */
		BMS_SoC_LimitAndUpdateWarning();

		/* Recalculate SoC percentage */
		if (BMS_SoC_State.NominalCapacityAh > 0.0f)
		{
			BMS_SoC_State.CurrentSoC = (BMS_SoC_State.RemainingCapacityAh / BMS_SoC_State.NominalCapacityAh) * 100.0f;
		}
		else
		{
		...
		/* Update warning flag after SoC change */
		BMS_SoC_State.LowSoCWarning = (BMS_SoC_State.CurrentSoC < BMS_SOC_WARNING_THRESHOLD);
    }
...
/**
 * @brief Get current SoC percentage.
 * @return Current SoC (0-100%).
 */
 ...
 * @return Remaining capacity in Ampere-hours.
 */
 ...
 * @param rawCurrent_mA Raw current from INA219 (mA)
 * @return Remaining hours; 1e6 if current=0, -1 if no capacity left while discharging.
 */
float32 BMS_SoC_GetRemainingHours(float32 rawCurrent_mA)
{
    ...
    if (mappedCurrent_A > 0.0f)  /* Discharging */
    {
        ...
        {
            remainingHours = -1.0f;  /* No capacity left */
        }
        else
        {
            remainingHours = BMS_SoC_State.RemainingCapacityAh / mappedCurrent_A;
        }
    }
    else if (mappedCurrent_A < 0.0f)  /* Charging */
    {
        float32 needed_Ah = BMS_SoC_State.NominalCapacityAh - BMS_SoC_State.RemainingCapacityAh;
        if (needed_Ah <= 0.0f)
        {
            remainingHours = 0.0f;  /* Already full */
        }
        else
        {
            remainingHours = needed_Ah / (-mappedCurrent_A);
        }
    }
    else  /* mappedCurrent_A == 0 */
    {
        remainingHours = 1e6f;  /* Infinite (no change) */
    }
 ...
 * @return TRUE if SoC < warning threshold, FALSE otherwise.
 */
...
/**
 * @brief Get the raw current range information for debugging.
 * @param minPtr Pointer to store minimum raw current (mA)
 * @param maxPtr Pointer to store maximum raw current (mA)
 * @param zeroPtr Pointer to store zero point current (mA)
 */
void BMS_SoC_GetCurrentRange(float32 *minPtr, float32 *maxPtr, float32 *zeroPtr)
{
    if (minPtr != NULL_PTR)
    {
    	*minPtr = BMS_RAW_CURRENT_MIN_mA;
    }
    if (maxPtr != NULL_PTR)
    {
    	*maxPtr = BMS_RAW_CURRENT_MAX_mA;
    }
    if (zeroPtr != NULL_PTR)
    {
    	*zeroPtr = BMS_CURRENT_ZERO_POINT_mA;
    }
}
...
/**
 * @brief Map raw unidirectional current to signed bidirectional current.
 * @param rawCurrent_mA Raw current from INA219 (mA), range: 2.4mA - 194mA
 * @return Mapped current in Amperes:
 *         - Positive: Discharging (raw > zero point)
 *         - Zero: No current (within dead zone)
 *         - Negative: Charging (raw < zero point)
 */
float32 BMS_SoC_MapCurrent(float32 rawCurrent_mA)
{
    float32 offsetCurrent_mA;
    float32 mappedCurrent_A;

    /* Clamp raw current to valid sensor range */
    if (rawCurrent_mA < BMS_RAW_CURRENT_MIN_mA)
    {
        rawCurrent_mA = BMS_RAW_CURRENT_MIN_mA;
    }
    if (rawCurrent_mA > BMS_RAW_CURRENT_MAX_mA)
    {
        rawCurrent_mA = BMS_RAW_CURRENT_MAX_mA;
    }

    /* Calculate offset from zero point */
    offsetCurrent_mA = (rawCurrent_mA > BMS_CURRENT_ZERO_POINT_mA) ?
    					(rawCurrent_mA - BMS_CURRENT_ZERO_POINT_mA) :
							-rawCurrent_mA;

    /* Apply dead zone to avoid noise flipping state */
    if (offsetCurrent_mA < BMS_CURRENT_DEAD_ZONE_mA &&
        offsetCurrent_mA > -BMS_CURRENT_DEAD_ZONE_mA)
    {
        offsetCurrent_mA = 0.0f;
    }

    //offsetCurrent_mA /= 1000.0f;

    /* Scale to output range [-1.0A, 1.0A] */
//    mappedCurrent_A = offsetCurrent_mA / BMS_CURRENT_MAX_RANGE_mA;
//
//    /* Clamp output to [-1.0, 1.0] */
//    if (mappedCurrent_A > BMS_MAPPED_CURRENT_MAX_A)
//    {
//        mappedCurrent_A = BMS_MAPPED_CURRENT_MAX_A;
//    }
//    if (mappedCurrent_A < -BMS_MAPPED_CURRENT_MAX_A)
//    {
//        mappedCurrent_A = -BMS_MAPPED_CURRENT_MAX_A;
//    }
//
//    return mappedCurrent_A;
    return offsetCurrent_mA;
}

/**
 * @brief Limit remaining capacity between 0 and nominal capacity, and update warning flag.
 */
static void BMS_SoC_LimitAndUpdateWarning(void)
{
    if (BMS_SoC_State.RemainingCapacityAh < 0.0f)
...
//{
//    float32 remain_Ah;      // Dung lượng còn lại (Ah)
//    float32 current_A;      // Dòng điện (A)
//    float32 empty_Ah;       // Dung lượng cần nạp (Ah)
//    float32 chg_A;          // Dòng sạc (A)
//    uint16  tte_min;
//    uint16  ttf_min;
//    uint8   valid_flags;
//    uint8   dummy_power_byte;  // Nếu CAN vẫn cần byte power, không thì bỏ
//
//    if (pOutSdu != NULL_PTR)
//    {
//        /* Công thức: remain_Ah = nominal_Ah × (soc_raw / 200)
//         * Vì soc_raw = 0-200, nominal_Ah = BMS_SOC_NOMINAL_CAPACITY_MAH / 1000
//         */
//        remain_Ah = ((float32)BMS_SOC_NOMINAL_CAPACITY_MAH / 1000.0f) *
//                    ((float32)soc_raw / (float32)BMS_SOC_FULL_RAW);
//
...
//
//        /* TTE: đang xả (curr > +5 mA) */
//        if (curr_mA > BMS_PRED_DISCHG_MIN_MA)
//        {
//            /* TTE (giờ) = remain_Ah / current_A
//             * ×60 để ra phút
//             */
//            tte_min = BmsSoc_SaturateMinutes((remain_Ah / current_A) * 60.0f);
//            valid_flags |= BMS_PRED_VALID_TTE;
//        }
//
//        /* TTF: đang sạc (curr < -5 mA) */
//        if (curr_mA < BMS_PRED_CHARGE_MAX_MA)
//        {
//            empty_Ah = ((float32)BMS_SOC_NOMINAL_CAPACITY_MAH / 1000.0f) - remain_Ah;
//            chg_A    = -current_A;  /* Dòng sạc dương */
//
//            if (chg_A > 0.001f)  /* Tránh chia cho 0 */
//            {
//                ttf_min = BmsSoc_SaturateMinutes((empty_Ah / chg_A) * 60.0f);
//                valid_flags |= BMS_PRED_VALID_TTF;
//            }
//        }
//
//        /* Nếu CAN 0x400 vẫn cần byte power (B4), gán 0 hoặc bỏ */
//        dummy_power_byte = 0U;
//
...
//        pOutSdu[4U] = dummy_power_byte;  /* Không còn power byte nữa */
...
////        if (curr_mA > BMS_PRED_DISCHG_MIN_MA)
////        {
////        	tte_min = BmsSoc_SaturateMinutes()
////        }
////
