...
#define SRTC_IP_SW_MAJOR_VERSION_C             3
...
/**
 * @brief   SRTC Status flag structure
 * @details Srtc_Ip_StatusFlagInterruptType

 */
typedef struct
{
    boolean alarmInterruptStatusFlag;
    boolean alarmInterruptEnableFlag;
    boolean invalidInterruptStatusFlag;
    boolean invalidInterruptEnableFlag;
    boolean overflowInterruptStatusFlag;
    boolean overflowInterruptEnableFlag;
} Srtc_Ip_StatusFlagInterruptType;
...
*                                      GLOBAL VARIABLES
==================================================================================================*/
#define GPT_START_SEC_VAR_INIT_32_NO_CACHEABLE
#include "Gpt_MemMap.h"
/**
...
#define GPT_STOP_SEC_VAR_INIT_32_NO_CACHEABLE
#include "Gpt_MemMap.h"

#define GPT_START_SEC_CONST_UNSPECIFIED
#include "Gpt_MemMap.h"
/** @brief Table of sRTC base pointers */
static RTC_Type * const sRtcBase[RTC_INSTANCE_COUNT] = IP_RTC_BASE_PTRS;
...
static inline void Srtc_Ip_InterruptHandler(uint8 instance, Srtc_Ip_StatusFlagInterruptType statusFlag);
#endif
...
static void Srtc_Ip_ClearChState(uint8 instance);
...
static void Srtc_Ip_ClearInvalidAndOverflowInterruptStatusFlags(uint8 instance);
/*==================================================================================================
*                                       LOCAL FUNCTIONS
==================================================================================================*/
/**
...
static void Srtc_Ip_ClearChState(uint8 instance)
{
    Srtc_Ip_u32ChState[instance].alarmInit          = FALSE;
    Srtc_Ip_u32ChState[instance].repetitionInterval = 0U;
    Srtc_Ip_u32ChState[instance].numberOfRepeats    = 0U;
    Srtc_Ip_u32ChState[instance].repeatForever      = FALSE;
    Srtc_Ip_u32ChState[instance].alarmIntEnable     = FALSE;
    Srtc_Ip_u32ChState[instance].alarmCallback      = NULL_PTR;
    Srtc_Ip_u32ChState[instance].callbackParams     = 0U;
    Srtc_Ip_u32ChState[instance].isAlarmTimeNew     = FALSE;
    Srtc_Ip_u32ChState[instance].chInit             = FALSE;
    Srtc_Ip_u32ChState[instance].callback           = NULL_PTR;
    Srtc_Ip_u32ChState[instance].callbackParam      = 0U;
}
...
#ifdef SRTC_0_CH_0_ISR_USED
/**
* @brief        Srtc_Ip_InterruptHandler.
*
...
static inline void Srtc_Ip_InterruptHandler(uint8 instance, Srtc_Ip_StatusFlagInterruptType statusFlag)
{
    ...
    uint8                   callbackParam;
    Srtc_Ip_ChannelModeType channelMode;

    alarmInterruptStatusFlag = statusFlag.alarmInterruptStatusFlag;
    alarmInterruptEnableFlag = statusFlag.alarmInterruptEnableFlag;
    invalidInterruptStatusFlag = statusFlag.invalidInterruptStatusFlag;
    invalidInterruptEnableFlag = statusFlag.invalidInterruptEnableFlag;
    overflowInterruptStatusFlag = statusFlag.overflowInterruptStatusFlag;
    overflowInterruptEnableFlag = statusFlag.overflowInterruptEnableFlag;

    if ((alarmInterruptStatusFlag && alarmInterruptEnableFlag) || (invalidInterruptStatusFlag && invalidInterruptEnableFlag) || (overflowInterruptStatusFlag && overflowInterruptEnableFlag))
    {
        callback        = Srtc_Ip_u32ChState[instance].callback;
        channelMode     = Srtc_Ip_u32ChState[instance].channelMode;
        callbackParam   = Srtc_Ip_u32ChState[instance].callbackParam;

        /* Check if channel mode is ONE-SHOT */
        if(SRTC_IP_CH_MODE_ONESHOT == channelMode)
        {
            (void)Srtc_Ip_StopCounter(instance);
        }

        /* Check if TAF and TAIE */
        if (alarmInterruptStatusFlag && alarmInterruptEnableFlag)
        {
            /* Process AlarmInterrupt */
            Srtc_Ip_ProcessAlarmInterrupt(instance);
        }

        if ((invalidInterruptStatusFlag && invalidInterruptEnableFlag) || (overflowInterruptStatusFlag && overflowInterruptEnableFlag))
        {
...
static void Srtc_Ip_ProcessAlarmInterrupt(uint8 instance)
{
    ...
    /* Set New Time Alarm and clear the Time Alarm Flag (TAF) too */
    ...
    else
    {
        ...
        if (Srtc_Ip_u32ChState[instance].alarmInit && alarmIntEnable)
        {
...
static void Srtc_Ip_ProcessInterrupt(uint8 instance)
{
    ...
    boolean                 chInit;
    Srtc_Ip_StatusFlagInterruptType statusFlag;
    ...
    SchM_Enter_Gpt_GPT_EXCLUSIVE_AREA_61();
    {
        ...
        alarmInterruptEnableFlag = Srtc_Ip_GetInterruptEnableFlag(instance, SRTC_IP_ALARM_INTERRUPT);
        invalidInterruptStatusFlag = Srtc_Ip_GetInterruptStatusFlag(instance, SRTC_IP_INVALID_INTERRUPT);
        invalidInterruptEnableFlag = Srtc_Ip_GetInterruptEnableFlag(instance, SRTC_IP_INVALID_INTERRUPT);
        overflowInterruptStatusFlag = Srtc_Ip_GetInterruptStatusFlag(instance, SRTC_IP_OVERFLOW_INTERRUPT);
        overflowInterruptEnableFlag = Srtc_Ip_GetInterruptEnableFlag(instance, SRTC_IP_OVERFLOW_INTERRUPT);

        statusFlag.alarmInterruptStatusFlag = alarmInterruptStatusFlag;
        statusFlag.alarmInterruptEnableFlag = alarmInterruptEnableFlag;
        statusFlag.invalidInterruptStatusFlag = invalidInterruptStatusFlag;
        statusFlag.invalidInterruptEnableFlag = invalidInterruptEnableFlag;
        statusFlag.overflowInterruptStatusFlag = overflowInterruptStatusFlag;
        statusFlag.overflowInterruptEnableFlag = overflowInterruptEnableFlag;
        ...
        if (TRUE == chInit)
        {
            ...
            if (alarmInterruptStatusFlag && alarmInterruptEnableFlag)
            {
                /* Clear Time Alarm Flag (TAF) by writing the TAR register.
            ...
            if ((invalidInterruptStatusFlag && invalidInterruptEnableFlag) || (overflowInterruptStatusFlag && overflowInterruptEnableFlag))
            {
                ...
                Srtc_Ip_ClearInvalidAndOverflowInterruptStatusFlags(instance);
            }
        }
        else
        {
    ...
    if (TRUE == chInit)
    {
        Srtc_Ip_InterruptHandler(instance, statusFlag);
    }
}
#endif
#if (STD_ON == SRTC_IP_ENABLE_LOCK_REGISTER_API)
...
static void Srtc_Ip_ClearInvalidAndOverflowInterruptStatusFlags(uint8 instance)
{
    boolean isTimerEnabled = Srtc_Ip_IsTimerEnabled(instance);
    /* Clear Software Reset */
    Srtc_Ip_SetSoftwareResetFlag(instance, FALSE);
    /* Disable time counter */
    Srtc_Ip_SetTimeCounterEnable(instance, FALSE);
    /* Clear the Time Invalid Flag (TIF) and Time Overflow Flag (TOF).
        these bits are cleared by writing the TSR register when the time counter is disabled. */
    Srtc_Ip_SetTimeSecondsRegVal(instance, 0U);
    /* Restore time counter */
    Srtc_Ip_SetTimeCounterEnable(instance, isTimerEnabled ? TRUE : FALSE);
}
/*==================================================================================================
*                                       GLOBAL FUNCTIONS
==================================================================================================*/
                                      ...
                                      uint32 * const seconds)
{
    ...
    const uint16 MONTH_DAYS[] = {0U, 0U, 31U, 59U, 90U, 120U, 151U, 181U, 212U, 243U, 273U, 304U, 334U};
...
void Srtc_Ip_DeInit(uint8 instance)
{
    ...
    Srtc_Ip_SetInterruptEnableFlag(instance, SRTC_IP_OVERFLOW_INTERRUPT, FALSE);
    Srtc_Ip_SetInterruptEnableFlag(instance, SRTC_IP_ALARM_INTERRUPT, FALSE);
    Srtc_Ip_SetInterruptEnableFlag(instance, SRTC_IP_SECONDS_INTERRUPT, FALSE);
    /* Clear the Time Invalid Flag (TIF) and Time Overflow Flag (TOF) by writing the TSR register when the time counter is disabled */
    Srtc_Ip_SetTimeSecondsRegVal(instance, 0U);
    /* Clear TAF by writing to the TAR register */
    Srtc_Ip_SetTimeAlarmRegVal(instance, 0U);
    /* Setup the RTC instance clock source out is DISABLED */
    Srtc_Ip_SelectClockOut(base, SRTC_IP_CLKOUT_DISABLED);

    /* Clear global variables */
    Srtc_Ip_u32TargetValue = 0U;
    /* Deallocate the state structure to avoid errors on reinitialization */
    Srtc_Ip_ClearChState(instance);
}
...
void Srtc_Ip_StartTimer(uint8 instance, uint32 value)
{
    ...
    /* Write alarm time into Time Alarm Register and clear the Time Alarm Flag (TAF) too */
...
*/
Srtc_Ip_StatusType Srtc_Ip_GetTimeDate(uint8 instance, Srtc_Ip_TimedateType * timeDate)
{
#if SRTC_IP_DEV_ERROR_DETECT == STD_ON
    DevAssert(NULL_PTR != timeDate);
    DevAssert(RTC_INSTANCE_COUNT > instance);
...
Srtc_Ip_StatusType Srtc_Ip_ConfigureLockRegister(uint8 instance,
                                                     const Srtc_Ip_LockRegConfigType * lockRegister)
{
    Srtc_Ip_StatusType status = SRTC_IP_ERROR;
    RTC_Type * base = sRtcBase[instance];
