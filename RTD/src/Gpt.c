...
#endif
#if (GPT_MULTIPARTITION_ENABLED == STD_ON)
    if (userId != configPtr->u32PartitionId)
    {
        (void)Det_ReportError\
        (\
            (uint16)GPT_MODULE_ID,\
...
#endif /*GPT_MULTIPARTITION_ENABLED*/
...
#endif
#if (GPT_MULTIPARTITION_ENABLED == STD_ON)
    if (userId != Gpt_Config[userId]->u32PartitionId)
    {
        (void)Det_ReportError\
        (\
            (uint16)GPT_MODULE_ID,\
...
void Gpt_Init(const Gpt_ConfigType * configPtr)
{
    ...
    if((Std_ReturnType)E_OK == Gpt_ValidateGlobalCall(GPT_INIT_ID, userId))
    {
        ...
        if((Std_ReturnType)E_OK == Gpt_ValidateParamPtrInit(GPT_INIT_ID, userId, configPtr))
        {
#endif
