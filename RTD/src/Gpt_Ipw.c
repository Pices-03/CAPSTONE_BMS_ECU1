...
*                                       LOCAL MACROS
==================================================================================================*/
#define MAX_VAL_32BIT   ((uint32)0xFFFFFFFFU)
...
static Gpt_ValueType Gpt_Ipw_FtmGetTimeElapsed(const Gpt_Ipw_HwChannelConfigType * pHwChannelConfig, Gpt_HwChannelInfoType * pReturnHwChannelInfo)
{
    ...
    do {
        previousCompareVal = compareValue;
        /* Read again compare register to be sure it's the right value */
        compareValue = Ftm_Gpt_Ip_GetCompareValue(pHwChannelConfig->instance, pHwChannelConfig->channel);
    } while ((++retries < MAX_RETRIES) && (compareValue != previousCompareVal));

    if (retries < MAX_RETRIES)
    {
        /* this is normal behavior: Calculate the elapsed time */
    ...
    {
        /* this is abnormal behavior - report the maximum value */
        returnValue = MAX_VAL_32BIT;
    }
